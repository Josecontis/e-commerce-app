...
//metodo che preleva l'intero snapshot
export const convertCollectionsSnapshotToMap = (collections) => {
	const transformedCollection = collections.docs.map((doc) => {
		const { title, items } = doc.data();

		return {
			routeName: encodeURI(title.toLowerCase()), //passa il titolo per assegnarlo all'indirizzo a cui farà riferimento
			id: doc.id,
			title,
			items
		};
	...
	//restituisce un array di matching con i titoli delle collection, a partire dallo stato inizaile {}
